...
describe('app', function(){
  ...
  it('should not .writeHead when !socket.writable', function(done){
    ...
    setImmediate(done); // setImmediate node非io的异步api之一

    request(app.listen())
    .get('/')
    .end(function(){});
  })
  // koa的env默认是process.env.NODE_ENV，如果process.env.NODE_ENV没有的话，koa env缺省为'development'
...
// app.toJSON() 方法返回 object 的字符串表示，主要用于调试。
describe('app.toJSON()', function(){
  // app.toJSON() 只打印三项内容，这三项的默认值是subdomainOffset=2，proxy为false，mocha测试下env为test
...
describe('app.use(fn)', function(){
  ...
  // koa use的中间件必须是generator function
...
// app.onerror(err)  koa应用级别的错误处理
describe('app.onerror(err)', function(){
  // app.onerror(err)只接受Error对象
  ...
  // 如果err.stack是空，打印错误信息err.toString()
